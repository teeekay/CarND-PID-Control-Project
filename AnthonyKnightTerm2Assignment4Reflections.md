...
[Link to Youtube video]("https://www.youtube.com/watch?v=BbUgvfqjlbA")



I found that I was able to use manual tuning to determine suitable PID parameters (Kp = 0.244, Ki = 0.0, and Kd = 8.91) that generally worked until the car throttle went above 0.65.  Ki was zero as there was no observable steering bias away from the center of the road.  

The results of tests completed with these PID parameter settings as the throttle rate was increased in subsequent runs show, in Figure 2, how the cumulative error rate in each run remained relatively constant until an inflection point near throttle setting of 0.65, beyond which the cumulative error metric increased significantly.  This suggests that the more the throttle rate was increased beyond this point, the sooner the car left the track - increasing the CTE measurements.
